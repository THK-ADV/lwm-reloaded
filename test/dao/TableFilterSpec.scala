...
class FakeLabworkTable(tag: Tag)
    extends Table[(UUID, UUID)](tag, "FAKE_LABWORK")
    with UniqueTable
    with LabworkIdTable {
  def * = (labwork, id)
}

class FakeLabelTable(tag: Tag)
    extends Table[(String, UUID)](tag, "FAKE_LABEL")
    with UniqueTable
    with LabelTable {
  def * = (label, id)
}

class FakeDateStartEndTable(tag: Tag)
    extends Table[(Date, Time, Time, UUID)](tag, "FAKE_DATESTARTEND")
    with UniqueTable
    with DateStartEndTable {
  def * = (date, start, end, id)
}

class FakeUserRefTable(tag: Tag)
    extends Table[(UUID, UUID)](tag, "FAKE_USERREF")
    with UniqueTable
    with UserIdTable {
...
class TableFilterSpec extends PostgresDbSpec with DateGenerator {
  ...
  private val labworks_ =
    populateLabworks(20)(semesters, courses.take(3), degrees)
  ...
  "A TableFilterSpec" should {
    ...
    "filter by course through labwork" in {
      ...
      runAsync(
        TableQuery[FakeLabworkTable].filter(courseFilter(c).apply).result
      ) { e =>
        e.map(_._1) should contain theSameElementsAs Seq(ll1(0).id, ll1(1).id)
      }
      runAsync(
        TableQuery[FakeLabworkTable]
          .filter(labworkFilter(UUID.randomUUID).apply)
          .result
      )(_ shouldBe empty)
    }
    ...
    "filter by label" in {
      ...
      runAsync(
        TableQuery[FakeLabelTable].filter(labelFilterEquals("dunno")).result
      )(_.map(_._1) shouldBe Seq("dunno"))
      runAsync(
        TableQuery[FakeLabelTable].filter(labelFilterEquals("fake")).result
      )(_.map(_._1) shouldBe empty)
      runAsync(
        TableQuery[FakeLabelTable].filter(labelFilterLike("fake")).result
      )(_.map(_._1) shouldBe Seq("fake yo", "fake to success"))
    }
    ...
    "filter by date" in {
      ...
      def make(
        ...
        (
          localDate(y, m, d).sqlDate,
          localTime(sh, sm, ss, 0).sqlTime,
          localTime(eh, em, es, 0).sqlTime,
          UUID.randomUUID
        )
      }
      ...
      runAsync(TableQuery[FakeDateStartEndTable].forceInsertAll(entries))(_ =>
        Unit
      )

      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(onDateFilter(localDate(2010, 1, 1).sqlDate))
          .result
      ) { result =>
      ...
      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(onDateFilter(localDate(2010, 2, 1).sqlDate))
          .result
      ) { result =>
      ...
      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(sinceFilter(localDate(2010, 5, 1).sqlDate))
          .result
      ) { result =>
      ...
      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(sinceFilter(localDate(2012, 5, 1).sqlDate))
          .result
      ) { result =>
        result shouldBe empty
      }
      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(untilFilter(localDate(2011, 3, 1).sqlDate))
          .result
      ) { result =>
      ...
      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(untilFilter(localDate(2014, 3, 1).sqlDate))
          .result
      ) { result =>
      ...
      runAsync(
        TableQuery[FakeDateStartEndTable]
          .filter(untilFilter(localDate(2009, 3, 1).sqlDate))
          .result
      ) { result =>
        result shouldBe empty
      }
    ...
    "filter by systemId through users" in {
      ...
      runAsync(
        TableQuery[FakeUserRefTable].filter(systemIdFilter("3")).result
      ) { result =>
        result shouldBe Seq(entries(3))
      }
      runAsync(
        TableQuery[FakeUserRefTable].filter(systemIdFilter("8")).result
      ) { result =>
        result shouldBe Seq(entries(8))
      }
      runAsync(
        TableQuery[FakeUserRefTable].filter(systemIdFilter("other")).result
      ) { result =>
        result shouldBe empty
      }
    }
  ...
  override protected def dependencies
      : profile.api.DBIOAction[Unit, profile.api.NoStream, Effect.Write] =
    DBIO.seq()
}
