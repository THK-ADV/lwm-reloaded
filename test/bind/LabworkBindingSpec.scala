...
class LabworkBindingSpec extends SesameDbSpec {
  ...
  val labwork = SesameLabwork("AP Praktikum", "AP Praktikum", SesameSemester.randomUUID, SesameCourse.randomUUID, SesameDegree.randomUUID)
  val labworkGraph = URI(SesameLabwork.generateUri(labwork)).a(lwm.Labwork)
    ...
    .--(lwm.degree).->-(labwork.degree)(ops, uuidRefBinder(SesameDegree.splitter))
  ...
  "A LabworkBinding" should {
    ...
    "return a labwork atom based on an RDF representation" in {
      ...
      val degree = SesameDegree("degree", "abbr")
