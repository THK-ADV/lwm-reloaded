...
class TimetableCRUDControllerSpec extends AbstractCRUDControllerSpec[TimetableProtocol, Timetable] {
  ...
  override val entityToFail: Timetable = Timetable(Labwork.randomUUID, Set.empty[TimetableEntry], DateTime.now, Set.empty[DateTime], 0, Timetable.randomUUID)

  override val entityToPass: Timetable = Timetable(Labwork.randomUUID, Set.empty[TimetableEntry], DateTime.now, Set.empty[DateTime], 0, Timetable.randomUUID)
  ...
  override val inputJson: JsValue = Json.obj(
    ...
    "blacklist" -> entityToPass.blacklist,
    "buffer" -> entityToPass.buffer
  )
}
