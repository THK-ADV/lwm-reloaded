...
"A AuthorityControllerSpec " should {
  ...
  contentAsJson (result1).asInstanceOf[JsArray].value foreach {
  auth =>
  expected1 contains auth shouldBe true
}

  contentAsJson (result2).asInstanceOf[JsArray].value foreach {
  auth =>
  expected2 contains auth shouldBe true
}

  contentAsJson (result3).asInstanceOf[JsArray].value foreach {
  auth =>
  expected3 contains auth shouldBe true
}

  contentAsJson (result4).asInstanceOf[JsArray].value foreach {
  auth =>
  expected4 contains auth shouldBe true
}

  contentAsJson (result5).asInstanceOf[JsArray].value foreach {
  auth =>
