...
class TimetableServiceSpec extends WordSpec with TestBaseDefinition {
  ...
  "A TimetableService" should {
    ...
    "extrapolate further entries based on frontend's timetable protocol template and assignment plan" in {
      ...
      val blacklist = profileWeek ++ christmas

      val protocol = TimetableProtocol(Labwork.randomUUID, tEntries, fd.parseDateTime("19/10/2015"), blacklist)
      ...
      result.entries.toVector.map(_.start).sorted.map(_.toString(ft))foreach println
    }
  }
}
