...
trait ResultOps { self: BaseController =>
  ...
  implicit class OptionResult[A](val future: Future[Option[A]]) {
    ...
    def jsonResult(idOfEntity: UUID)(implicit writes: Writes[A], executor: ExecutionContext): Future[Result] = future.map { maybeA =>
      maybeA.fold(internalServerError(s"cannot update or delete $idOfEntity")) { a =>
        Ok(Json.obj("status" -> "OK", "delete|update" -> Json.toJson(a)))
      }
    }.recover {
      case NonFatal(e) => internalServerError(e)
    }
  ...
  implicit class CreatedResult[A](val future: Future[A]) {
    def jsonResult(implicit writes: Writes[A], executor: ExecutionContext): Future[Result] = future.map(a => Created(Json.toJson(a))).recover {
      case NonFatal(e) => internalServerError(e)
    }
